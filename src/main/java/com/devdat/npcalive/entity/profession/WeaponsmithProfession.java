...
public class WeaponsmithProfession implements ProfessionLogic {
    ...
    public BlockPos getTargetPosition(NpcEntity npc, BlockPos workPos) {
        ...
        // 2. Posicionamiento seguro alrededor del afilador
    ...
    public void tickWork(NpcEntity npc, BlockPos targetPos, BlockPos workPos, int workTimer) {
        ...
        // Fija la mirada en el afilador
        npc.getLookControl().setLookAt(workPos.getX() + 0.5D, workPos.getY() + 0.5D, workPos.getZ() + 0.5D, 30.0F, 30.0F);

        // Efectos visuales de chispas en el afilador
        if (workTimer % 40 == 0) {
            npc.swing(InteractionHand.MAIN_HAND, true);
            // Sonido del aldeano maestro de armas trabajando
            serverLevel.playSound(null, workPos, SoundEvents.VILLAGER_WORK_WEAPONSMITH, SoundSource.BLOCKS, 0.5F, serverLevel.getRandom().nextFloat() * 0.2F + 0.9F);

            // Partículas de chispa de afilador / fuego artificial pequeño o crítico
    ...
    public void performWork(NpcEntity npc, BlockPos targetPos, BlockPos workPos) {
        ...
        // Generación de recompensas de armas
        npc.swing(InteractionHand.MAIN_HAND, true);
        giveWeaponsmithReward(npc, serverLevel, workPos);
    }

    private void giveWeaponsmithReward(NpcEntity npc, ServerLevel serverLevel, BlockPos pos) {
        float roll = serverLevel.getRandom().nextFloat();
        ItemStack reward;

        if (roll < 0.45F) { // Carbón o lingotes
            reward = new ItemStack(Items.IRON_INGOT, 1 + serverLevel.getRandom().nextInt(2));
        } else if (roll < 0.70F) { // Armas de hierro (Espada o Hacha de combate)
            Item[] weapons = {Items.IRON_SWORD, Items.IRON_AXE};
            reward = new ItemStack(weapons[serverLevel.getRandom().nextInt(weapons.length)], 1);
        } else if (roll < 0.88F) { // Pedernal y acero o campana
            reward = serverLevel.getRandom().nextBoolean()
                    ? new ItemStack(Items.FLINT_AND_STEEL, 1)
                    : new ItemStack(Items.BELL, 1);
        } else if (roll < 0.98F) { // Espada de diamante (recompensa épica)
            reward = new ItemStack(Items.DIAMOND_SWORD, 1);
        } else { // Esmeralda
            reward = new ItemStack(Items.EMERALD, 1);
        }

        npc.addItemToBackpack(reward);
        serverLevel.playSound(null, pos, SoundEvents.GRINDSTONE_USE, SoundSource.BLOCKS, 0.8F, 1.0F);
    }
